...
class GameSessionAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        res = self.client.post(reverse('bingoroom-list'))
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.room = res.data

        # Player entra na sala
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.player_token)
        res = self.client.post(reverse('join-room'), {"room": self.room["id"]})
        self.assertEqual(res.status_code, status.HTTP_200_OK)

        # Player cria cartela
        res = self.client.post(reverse('bingocard-list'), {"room": self.room["id"]})
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.card = res.data

        # Host cria sessão
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.host_token)
        res = self.client.post(reverse('game-session-list'), {"room": self.room["id"]})
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
    ...
    def test_end_game_session_manually(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertFalse(GameSession.objects.get(id=self.session["id"]).is_active)
    ...
    def test_block_second_winner(self):
        ...
        # Tenta validar de novo e deve falhar
        response = self.client.post(reverse('game-session-validate-bingo', args=[self.session["id"]]))
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
